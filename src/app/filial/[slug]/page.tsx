      ...
      <TeacherSearch>
        {teachers.map((t, i) => (
          <Link
            ...
            className="card card-hover animate-in flex items-center gap-4 p-4"
            style={{ animationDelay: `${Math.min(i, 12) * 40}ms` }}
          >
            <Avatar name={t.full_name} src={t.photo_url} size={56} />
            <div className="min-w-0 flex-1">
              <h3 className="truncate font-bold text-[color:var(--ink)]">
                {t.full_name}
              </h3>
              {t.subject && (
                <p className="muted truncate text-xs">{t.subject}</p>
              )}
            </div>
            {voted.has(t.id) ? (
